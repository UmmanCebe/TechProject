...
namespace Tests
{
            ...
            var events = new List<EventResponseDto>
    {
        ...
        new EventResponseDto { Id = Guid.NewGuid(), Title = "Event 2" }
            ...
            _mockEventService
                .Setup(service => service.GetListAsync(null, null, true, false, false, default))
                .ReturnsAsync(events);
        ...
        public async Task Get_ShouldReturnOkResultWithEvent_WhenEventExists()
        {
            ...
            _mockEventService.Setup(service => service.GetAsync(e => e.Id == eventId, true, false, true, default))
